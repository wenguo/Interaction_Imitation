...
extern int e_proximity_filtered[8];

static int init_value_ir[8] = {0,0,0,0,0,0,0,0};
extern unsigned char ir_pulse_mask;
...
int e_get_prox_filtered(unsigned int sensor_number)
{
	if (sensor_number > 7)
		return 0;
	else
		return e_proximity_filtered[sensor_number];
}
...
void e_set_ir_pulse_mask(unsigned int i, unsigned int value)
{
    if(value)
        ir_pulse_mask |= 1<<i;
    else
